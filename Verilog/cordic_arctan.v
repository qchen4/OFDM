...
module cordic_arctan#(
	...
	output [C_ANGLE_WIDTH - 1 : 0] O_theta
);

wire [C_ANGLE_WIDTH-1:0] W_arctan_table[0:C_ITER_NUM-1] ; //16S13
...
assign W_y[0] = I_y;
assign W_z[0] = 'd0;
assign W_d[0] = I_y[C_DATA_WIDTH - 1];
   ...
   for(i = 0; i < C_ITER_NUM; i = i + 1) begin
	   cordic_shift
		...
		u(
			...
			.I_z(W_z[i]),
			.I_d(W_d[i]),
			.I_arctan_table_in(W_arctan_table[i]), //16S13
			.O_x(W_x[i+1]),
			.O_y(W_y[i+1]),
			.O_z(W_z[i+1]),
			.O_d(W_d[i+1])
