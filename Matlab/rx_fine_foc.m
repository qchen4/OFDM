...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function rx_fine_foc_out = rx_fine_foc(rx_data_in,sample_rate)
...
quan_22S13_pattern = quantizer('fixed','round','saturate',[22 13]);
quan_16S7_pattern = quantizer('fixed','round','saturate',[16 7]);
...
for k = 1:L
	...
	cmult_out = quantize(quan_10S7_pattern,cmult_out);
	Pk = Pk + cmult_out;  
end
Pk = quantize(quan_16S7_pattern,Pk);
...
if((abs_Pk_max > 0 && abs_Pk_max <= 1))
    Pk_22S13 = quantize(quan_22S13_pattern,Pk); %22S13
elseif((abs_Pk_max > 1 && abs_Pk_max <= 2))
    Pk_22S13 = quantize(quan_22S13_pattern,Pk/2); 
elseif((abs_Pk_max > 2 && abs_Pk_max <= 4))
    Pk_22S13 = quantize(quan_22S13_pattern,Pk/4); 
elseif((abs_Pk_max > 4 && abs_Pk_max <= 8))
    Pk_22S13 = quantize(quan_22S13_pattern,Pk/8);    
elseif((abs_Pk_max > 8 && abs_Pk_max <= 16))
    Pk_22S13 = quantize(quan_22S13_pattern,Pk/16);
elseif((abs_Pk_max > 16 && abs_Pk_max <= 32))
    Pk_22S13 = quantize(quan_22S13_pattern,Pk/32); 
else
    Pk_22S13 = quantize(quan_22S13_pattern,Pk);      
end

% 22S13 -> 16S13
Pk_16S13 = quantize(quan_16S13_pattern,Pk_22S13);
...
fine_estimate_freq = Pk_phi*scale_factor;
quan_22S0_pattern = quantizer('fixed','round','saturate',[22 0]);
fine_estimate_freq = quantize(quan_22S0_pattern,fine_estimate_freq);
fprintf('fine_estimate_freq = %d\n',fine_estimate_freq)

% 使用移位的方式将Pk_phi除以64得到相位累加因子
Pk_phi_div64 = fine_foc_phi_div64(Pk_phi,[16 13]);
Pk_phi_div64 = quantize(quan_16S13_pattern,Pk_phi_div64);
...
for i=1:length(rx_data_in)
    ...
    acc_phase = -Pk_phi_div64 + acc_phase;
...
rx_fine_foc_out = rx_data_in .* comp_item;
rx_fine_foc_out = quantize(quan_9S7_pattern,rx_fine_foc_out);
rx_fine_foc_out = quantize(quan_8S7_pattern_floor,rx_fine_foc_out/2);
print_txt(real(rx_fine_foc_out),[8 7],'./verify_txt/rx_fine_foc_out_i.txt');
print_txt(imag(rx_fine_foc_out),[8 7],'./verify_txt/rx_fine_foc_out_q.txt');

% 画图
figure('Name','rx_fine_foc');
subplot(2,2,1)
plot(real(rx_data_in(1:80)),'k')
title('Plot rx\_data\_in\_i')
xlabel('rx\_data\_in\_i')
ylabel('Magnitude')

subplot(2,2,2)
plot(imag(rx_data_in(1:80)),'k')
title('Plot rx\_data\_in\_q')
xlabel('rx\_data\_in\_q')
ylabel('Magnitude')

subplot(2,2,3)
plot(real(rx_fine_foc_out(1:80)),'k')
title('Plot rx\_fine\_foc\_out\_i')
xlabel('rx\_data\_in\_i')
ylabel('Magnitude')

subplot(2,2,4)
plot(imag(rx_fine_foc_out(1:80)),'k')
title('Plot rx\_fine\_foc\_out\_q')
xlabel('rx\_data\_in\_q')
ylabel('Magnitude')

print(gcf, 'rx_fine_foc.png', '-dpng', '-r600');
end
